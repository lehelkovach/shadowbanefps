...
| --- | --- | --- | --- |
| **Human** | Ironbrand, Ashwake, Dawnward, Wallwarden | **ShadowKight** pack in `Content/ShadowKight/` (wired) · [dReal Warrior Fantasy Character](https://www.fab.com/listings/ec2ce124-b60d-4159-bc1b-989beff7ae4b) (Fab freebie) · UE5 mannequin · [MetaHuman](https://www.unrealengine.com/en-US/metahuman) presets | Default dogfood: Manny Greystone A→B→C combo (`sb.Melee.ComboWindowSec` 0.5 post-anim; LMB bank; RMB cancel). `sb.Hero.UseShadowKight 1` → SK — see `README_MannyMelee.md` / `Content/ShadowKight/README.txt`. SK skeleton ≠ UE5 mannequin |
...
**ShadowKight:** vendored under `Content/ShadowKight/`. Soft refs in `SBHeroSkinPaths` + `sb.Hero.UseShadowKight` (default **0** = Manny combat dogfood; set **1** for SK). With CVar 1, Human race (and substrings `shadowkight`/`shadowknight`) load `ShadowknightUE_SK` + preferred `ABP_SK_Melee` / `Anim_ShadowKight`. Melee LMB plays `AM_SK_LibSwing_01` (fallback `AM_SK_SwordSwing_01`). With CVar 0, Manny Greystone **A→B→C** combo on `ABP_Manny` DefaultSlot (`sb.Melee.ComboWindowSec` = 0.5 after anim ends; LMB during anim banks next letter; RMB cancels swing). See `Content/Characters/Mannequins/Animations/Combat/README_MannyMelee.md`.
